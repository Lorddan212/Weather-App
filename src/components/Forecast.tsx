...
const ForecastDay: React.FC<ForecastDayProps> = ({ 
  ...
  return (
    <Card 
      className={`overflow-hidden transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 fill-mode-both ${
            ...
            <div className="pt-3">
              ...
              {day.forecasts.length > 0 && (
                <div className="mt-3 pt-3">
                  <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wider">Hourly</p>
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-muted">
                    {day.forecasts.slice(0, 8).map((forecast, idx) => (
                      <div 
                        key={idx}
                        className="flex-shrink-0 flex flex-col items-center p-2 rounded-lg bg-muted/30 min-w-[60px]"
                      >
...
export const Forecast: React.FC<ForecastProps> = ({ forecast, unit }) => {
  ...
  return (
    <div className="space-y-3 text-foreground">
      <div className="flex items-center justify-between rounded-xl bg-background/95 px-4 py-3 text-foreground shadow-xs backdrop-blur-sm">
