...
const WeatherMetric: React.FC<WeatherMetricProps> = ({ icon, label, value, delay = 0 }) => (
  <div 
    className="flex items-center gap-3 rounded-lg bg-background/95 p-3 text-foreground shadow-xs backdrop-blur-sm transition-all duration-300 hover:bg-background animate-in fade-in slide-in-from-bottom-4 fill-mode-both"
...
export const CurrentWeather: React.FC<CurrentWeatherProps> = ({ weather, unit }) => {
  ...
  return (
    <div className="space-y-6 text-foreground">
      {/* Main Weather Card */}
      <Card className="overflow-hidden bg-gradient-to-br from-card/95 to-background/95 backdrop-blur-xl shadow-2xl">
              ...
              <div className="flex items-center gap-4 animate-in fade-in slide-in-from-left-4 duration-500 delay-100">
                ...
                <div className="space-y-1">
                  <WeatherConditionIcon
                    ...
                    strokeWidth={1.6}
                  />
                  <p className="text-sm capitalize text-muted-foreground font-semibold">
                    {weather.description}
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-4 text-sm font-medium text-muted-foreground animate-in fade-in slide-in-from-left-4 duration-500 delay-200">
